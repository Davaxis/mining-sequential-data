...
# WD <- getwd()
#WD <- "/home/mokot/FRI/InteligentniSistemi/assignment02"
#WD <- '/home/david/programiranje/is/as2'
#if (!is.null(WD))
#  setwd(WD)
...
get_clean_data <- function(data, whole_words = TRUE) {
  ...
  corpus  <-
    tm_map(corpus , removeWords, stopwords('english')) # remove stopwordsd
  corpus  <- tm_map(corpus , removeNumbers) # remove numbers
  corpus  <- tm_map(corpus , removePunctuation) # remove punctation
  corpus <-
...
#  Rtsne(
#    mat_train,
#    perplexity = 1500,tsne_component_two <- as.numeric(tsne_train[, 2])
    # visualize the possible separation in 2D orthogonal space
...
# returns f1 score of a model
CF1 <- function(observed, predicted) {
  cm <- confusionMatrix(predicted, observed, mode = "everything", positive="1")
...
ca_nn
f1_nn <- CF1(observed_nn, predicted_nn)
f1_nn
ca_ba <- CA(observed_ba, predicted_ba)
ca_ba
f1_ba <- CF1(observed_ba, factor(as.integer(predicted_ba)))
f1_ba
...
ca_bo
f1_bo <- CF1(observed_bo, factor(as.integer(predicted_bo)))
f1_bo
...
f1_scores <- c(f1_knn, f1_dt, f1_nb, f1_svm, f1_nn, f1_ba, f1_rf, f1_bo)
combined <- c(ca_knn, f1_knn, ca_dt, f1_dt, ca_nb, f1_nb, ca_svm, f1_svm, ca_nn, f1_nn, ca_ba, f1_ba, ca_rf, f1_rf, ca_bo, f1_bo)
algorithm_names <- c("KNN", "DT", "NB", "SVM", "NN", "BA", "RF", "BO")
conclusion <- data.frame(performances, f1_scores, algorithm_names)
conclusion <-
  conclusion[rev(order(conclusion$performances)), ]
rownames(conclusion) <- NULL
conclusion
conclusion_by_f1 <- conclusion[rev(order(conclusion$f1_scores)), ]
conclusion_by_f1

graph_data <- data.frame(type=rep(c("accuracy", "F1 score"), 2), performances=combined, algorithm_names=rep(algorithm_names, each=2))
graph_data

# plot the results
p<-ggplot(data=graph_data, aes(x=algorithm_names, y=performances, fill=type)) +
  geom_bar(stat="identity", position=position_dodge())
p

graph_data <- data.frame(differences=abs(performances - f1_scores), algorithm_names)
graph_data

p<-ggplot(data=graph_data, aes(x=algorithm_names, y=differences)) +
